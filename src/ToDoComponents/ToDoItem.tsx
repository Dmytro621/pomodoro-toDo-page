        ...
        <div className={`task ${todo.completed ? 'completed': ''} `}>
            ...
            <div className="task-managment">
                <label className="checkbox" >
                    <input type="checkbox" className="checkbox__input" checked = {todo.completed} onChange={() => dispatch(toggleTask(todo.id))}/>
                    <span className="checkbox__box" aria-hidden="true"></span>
                </label>
                <button onClick={() => dispatch(deleteTask(todo.id))}></button>
            </div>
        </div>
    )
}
